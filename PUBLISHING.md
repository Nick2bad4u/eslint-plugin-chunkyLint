...
Use GitHub Actions > Manual Publish to bump `patch`/`minor`/`major` or specify exact version, with optional dry run. The workflow runs type-check, lint, tests, then build before publishing, publishes to npm, and creates the matching GitHub release using notes generated by `git-cliff`.
