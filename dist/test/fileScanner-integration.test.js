...
const createMockESLintWithIgnorePatterns = () => ({
    ...
    calculateConfigForFile: vi.fn().mockRejectedValue(new Error("Config error")), // This will trigger lines 147-152
    isPathIgnored: vi.fn().mockResolvedValue(false),
    lintFiles: vi.fn(),
    lintText: vi.fn(),
    findConfigFile: vi.fn(),
...
describe("FileScanner Coverage Integration Tests", () => {
    ...
    it("should process ESLint ignore patterns when found (covers lines 138-143)", async () => {
        ...
        vi.doMock("eslint", () => ({
            ESLint: vi.fn().mockImplementation(() => createMockESLintWithIgnorePatterns())
        }));
        // Re-import to get fresh module with new mock
        const { FileScanner: FreshFileScanner } = await import("../lib/fileScanner.js"), freshScanner = new FreshFileScanner(mockLogger), files = await freshScanner.scanFiles({
    ...
    it("should handle config calculation errors (covers lines 147-152)", async () => {
        ...
        vi.doMock("eslint", () => ({
            ESLint: vi.fn().mockImplementation(() => createMockESLintWithConfigError())
        }));
        // Re-import to get fresh module with new mock
        const { FileScanner: FreshFileScanner } = await import("../lib/fileScanner.js"), freshScanner = new FreshFileScanner(mockLogger), files = await freshScanner.scanFiles({
            config: "./eslint.config.js"
    ...
    it("should handle files ignored by ESLint (covers lines 170-171)", async () => {
        ...
        vi.doMock("eslint", () => ({
            ESLint: vi.fn().mockImplementation(() => createMockESLintWithIgnoredFiles())
        }));
        // Re-import to get fresh module with new mock
        const { FileScanner: FreshFileScanner } = await import("../lib/fileScanner.js"), freshScanner = new FreshFileScanner(mockLogger), files = await freshScanner.scanFiles({
            config: "./eslint.config.js"
    ...
    it("should handle path check errors (covers lines 174-179)", async () => {
        ...
        vi.doMock("eslint", () => ({
            ESLint: vi.fn().mockImplementation(() => createMockESLintWithPathError())
        }));
        // Re-import to get fresh module with new mock
        const { FileScanner: FreshFileScanner } = await import("../lib/fileScanner.js"), freshScanner = new FreshFileScanner(mockLogger), files = await freshScanner.scanFiles({
            config: "./eslint.config.js"
