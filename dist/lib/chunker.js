...
export class ESLintChunker {
    ...
    async run(progressCallback) {
        ...
        try {
            ...
            const chunks = this.fileScanner.chunkFiles(files, this.options.size);
            this.logger.info(`Processing ${files.length} files in ${chunks.length} chunks of ~${this.options.size} files each`);
            // Process chunks
            const chunkResults = await this.processChunks(chunks, progressCallback), endTime = performance.now(), stats = this.createStats(chunkResults, files.length, endTime - startTime);
    ...
    async processChunks(chunks, progressCallback) {
        const results = [], limit = pLimit(this.options.concurrency), 
        // Process chunks with controlled concurrency
        chunkPromises = chunks.map((chunk, index) => limit(() => this.processChunk(chunk, index)));
    ...
    async processChunk(files, chunkIndex) {
        const startTime = performance.now();
        this.logger.verbose(`Processing chunk ${chunkIndex + 1} with ${files.length} files`);
        try {
            ...
            const errorCount = results.reduce((sum, result) => sum + result.errorCount, 0), warningCount = results.reduce((sum, result) => sum + result.warningCount, 0), fixedCount = results.reduce((sum, result) => sum + (result.output ? 1 : 0), 0), processingTime = performance.now() - startTime;
            return {
                chunkIndex,
                files,
                success: true,
    ...
    createStats(chunkResults, totalFiles, totalTime) {
        const filesWithErrors = chunkResults.reduce((sum, chunk) => sum + (chunk.errorCount > 0 ? chunk.files.length : 0), 0), filesWithWarnings = chunkResults.reduce((sum, chunk) => sum + (chunk.warningCount > 0 ? chunk.files.length : 0), 0), filesFixed = chunkResults.reduce((sum, chunk) => sum + chunk.fixedCount, 0), failedChunks = chunkResults.filter((chunk) => !chunk.success).length;
        return {
            totalFiles,
    ...
    logChunkProgress(result, completed, total) {
        const progress = Math.round((completed / total) * 100), statusIcon = result.success ? "✅" : "❌";
     ...
     */
    logFinalStats(stats) {
        const totalTime = Math.round(stats.totalTime), avgTimePerFile = Math.round(stats.totalTime / stats.totalFiles);
        this.logger.info("");
        this.logger.info(chalk.green("✅ ESLint chunker completed!"));
        this.logger.info(`📊 Processed ${stats.totalFiles} files in ${stats.totalChunks} chunks`);
        this.logger.info(`⏱️  Total time: ${totalTime}ms (~${avgTimePerFile}ms per file)`);
        if (stats.filesWithErrors > 0) {
            this.logger.info(chalk.red(`❌ Files with errors: ${stats.filesWithErrors}`));
        }
        if (stats.filesWithWarnings > 0) {
            this.logger.info(chalk.yellow(`⚠️  Files with warnings: ${stats.filesWithWarnings}`));
        }
        if (stats.filesFixed > 0) {
            this.logger.info(chalk.green(`🔧 Files fixed: ${stats.filesFixed}`));
        }
        if (stats.failedChunks > 0) {
            this.logger.info(chalk.red(`💥 Failed chunks: ${stats.failedChunks}`));
        }
    }
    /**
